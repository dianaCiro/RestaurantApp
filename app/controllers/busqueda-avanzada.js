  ...
  actions: {
    filterAdvanced(email, description){
      ...
      if (email != '' && email != undefined) {
        ...
        filtered = this.get('content').filter((item,index,self)=>item.get('email').includes(email));
        if(description != '' && description != undefined){
          console.log(TAG+"description is not empty "+description);
          filtered = filtered.filter((item,index,self)=>item.get('description').includes(description));
        }
        return filtered;
      }
        ...
        return this.get('content').filter((item,index,self)=>item.get('description').includes(description));
      }
      else{
